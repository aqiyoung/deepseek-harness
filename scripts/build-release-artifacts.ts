...
function normalizeIsoUtcTimestamp(raw: string): string {
  ...
  if (!isoUtcTimestampPattern.test(timestamp)) {
    throw new Error("DSH_BUILD_TIMESTAMP must be an ISO-8601 UTC timestamp");
  }
  ...
  if (normalized.slice(0, 19) !== timestamp.slice(0, 19)) {
    throw new Error("DSH_BUILD_TIMESTAMP must be a valid ISO-8601 UTC timestamp");
  }
  return normalized;
}
...
export function resolveAndroidBuildMetadata(
  ...
  const configuredTimestamp = env.DSH_BUILD_TIMESTAMP?.trim();
...
export function androidBuildMetadataGradleArgs(metadata: AndroidBuildMetadata): string[] {
  return [
    `-PdeepseekHarnessBuildCommit=${metadata.commit}`,
    `-PdeepseekHarnessBuildTimestamp=${metadata.timestamp}`,
  ];
}
  ...
  return [
    ...
    },
    {
      ...
      sourcePath: join(
        ...
        "thirdParty",
        "release",
        `dsh-${versionName}-thirdParty-release.apk`,
      ),
    },
  ];
}
...
function main() {
  ...
  for (const artifact of artifacts) {
    const outputPath = join(
      releaseOutputDir,
      `dsh-${version.canonicalVersion}-${artifact.flavorName}-release.${artifact.kind}`,
    );
