...
- Bundled license files live in `apps/android/THIRD_PARTY_LICENSES/DeepSeek Harness/licenses/`.
...
- Do not add DeepSeek Harness, DeepSeek Harness Foundation, or other first-party/self-owned license entries. The screen is for third-party/open-source dependency acknowledgements.
- When adding, removing, or upgrading Android dependencies, audit whether `apps/android/THIRD_PARTY_LICENSES/DeepSeek Harness/licenses/` needs updates. Exclude dependencies owned by DeepSeek Harness Foundation from the published license list.
...
- When changing license loading or presentation, update `apps/android/app/src/test/java/ai/DeepSeek Harness/app/AndroidLicenseNoticesTest.kt`, then run focused Android validation.
