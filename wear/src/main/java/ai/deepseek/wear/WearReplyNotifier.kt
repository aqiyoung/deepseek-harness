...
internal class WearReplyNotifier(
  ...
  fun show(inbound: WearInboundEvent) {
    ...
    val agent = Person.Builder().setName("DeepSeekHarness").build()
    val notification =
      NotificationCompat
...
internal fun replyNotificationTag(
  ...
  return "ai.deepseek.wear.NOTIFICATION.${sha256("$sessionKey\u0000$messageIdentity")}"
}

internal fun replyPendingIntentAction(
  sessionKey: String,
  notificationTag: String,
): String = "ai.deepseek.wear.REPLY.${sha256("$sessionKey\u0000$notificationTag")}"
...
private const val LOG_TAG = "DeepSeekHarnessWear"
