package ai.deepseek.harness.accessibility
...
class AccessibilityActionExecutor internal constructor(
  private val connectionProvider: () -> AccessibilityServiceConnection<DeepSeekHarnessAccessibilityService> = {
    DeepSeekHarnessAccessibilityService.connection.value
  },
  private val captureSnapshot: (DeepSeekHarnessAccessibilityService) -> AccessibilitySnapshotCapture =
    AccessibilitySnapshotter()::capture,
  private val foregroundPackageProvider: (DeepSeekHarnessAccessibilityService) -> String? =
    DeepSeekHarnessAccessibilityService::foregroundPackageName,
  private val uiEpochProvider: () -> Long = { DeepSeekHarnessAccessibilityService.uiEpoch },
) : AutoCloseable {
  ...
  private fun coordinateGesturePreflight(service: DeepSeekHarnessAccessibilityService): ActionResult? {
  ...
  private fun nodeActionPackagePreflight(service: DeepSeekHarnessAccessibilityService): ActionResult? {
    val expectedPackage = generation.packageName
    val currentPackage = foregroundPackageProvider(service)
    if (expectedPackage == null || currentPackage == null || expectedPackage != currentPackage) {
      return ActionResult(
  ...
  private suspend fun dispatchGesture(
    service: DeepSeekHarnessAccessibilityService,
