...
import ai.deepseek.harness.gateway.testDeviceIdentityStore
...
class InvokeDispatcherTest {
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(DeepSeekHarnessSmsCommand.Search.rawValue, "not-json")
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(DeepSeekHarnessSmsCommand.Send.rawValue, """{"to":"+15551234567","message":"hi"}""")
  ...
  fun handleInvoke_blocksCameraCommandsWhenCameraDisabled() =
    runTest {
      val result = newDispatcher(cameraEnabled = false).handleInvoke(DeepSeekHarnessCameraCommand.List.rawValue, null)
  ...
  fun handleInvoke_blocksLocationCommandWhenLocationDisabled() =
    runTest {
      val result = newDispatcher(locationEnabled = false).handleInvoke(DeepSeekHarnessLocationCommand.Get.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(installedAppsSharingEnabled = false)
          .handleInvoke(DeepSeekHarnessDeviceCommand.Apps.rawValue, """{"limit":1}""")
    ...
    runTest {
      val result =
        newDispatcher(motionActivityAvailable = false)
          .handleInvoke(DeepSeekHarnessMotionCommand.Activity.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(motionPedometerAvailable = false)
          .handleInvoke(DeepSeekHarnessMotionCommand.Pedometer.rawValue, null)
  ...
  fun handleInvoke_blocksCallLogWhenUnavailable() =
    runTest {
      val result =
        newDispatcher(callLogAvailable = false).handleInvoke(DeepSeekHarnessCallLogCommand.Search.rawValue, null)
  ...
  fun handleInvoke_blocksPhotosWhenUnavailable() =
    runTest {
      val result = newDispatcher(photosAvailable = false).handleInvoke(DeepSeekHarnessPhotosCommand.Latest.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(mobileUiAvailable = false)
          .handleInvoke(DeepSeekHarnessMobileUiCommand.Observe.rawValue, null)
  ...
  fun handleInvoke_routesTalkPttCommands() =
    runTest {
      ...
      val start = dispatcher.handleInvoke(DeepSeekHarnessTalkCommand.PttStart.rawValue, null)
      val stop = dispatcher.handleInvoke(DeepSeekHarnessTalkCommand.PttStop.rawValue, null)
      val cancel = dispatcher.handleInvoke(DeepSeekHarnessTalkCommand.PttCancel.rawValue, null)
      val once = dispatcher.handleInvoke(DeepSeekHarnessTalkCommand.PttOnce.rawValue, null)
  ...
  fun handleInvoke_presentAndHideDriveTheShellOwnedCanvasState() =
    runTest {
      ...
      val hide = dispatcher.handleInvoke(DeepSeekHarnessCanvasCommand.Hide.rawValue, null)
    ...
    runTest {
      ...
      val result =
        newDispatcher(isForeground = false, canvas = canvas)
          .handleInvoke(DeepSeekHarnessCanvasCommand.Present.rawValue, """{"url":"https://example.com"}""")
