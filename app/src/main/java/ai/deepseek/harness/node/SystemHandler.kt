...
import ai.deepseek.harness.mainActivityPendingIntent
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" ->
          Triple("passive", NotificationManager.IMPORTANCE_LOW, nativeString("DeepSeekHarness Passive"))
        "timesensitive" ->
          Triple(
            "timesensitive",
            NotificationManager.IMPORTANCE_HIGH,
            nativeString("DeepSeekHarness Time Sensitive"),
          )
        else ->
          Triple("active", NotificationManager.IMPORTANCE_DEFAULT, nativeString("DeepSeekHarness Active"))
      }
