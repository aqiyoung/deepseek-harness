...
/**
 * 统一更新检查引擎（Kotlin 版）—— 对齐 openlist-android / sanyelive / FeiNiuMusic / synapse
 * 共用的 app_update_core 行为。
 *
 * 解决的真实问题（与其一一对应）：
 *  1) 国内 / 移动宽带直连 api.github.com 被墙 → 代理链 [gh-llkk.cc → gh-proxy.com → 直连]
 *     逐层尝试，任一层 403 / 超时 / 拿到 HTML 都静默跳下一层，不会因代理抽风整体失败。
 *  2) 只比 tag_name vs BuildConfig.versionName，不依赖 APK 文件名格式。
...
class GitHubUpdateEngine(private val config: UpdateConfig) {
  private val apiHeaders =
    mapOf(
      ...
      "User-Agent" to "deepseek-harness",
  ...
  suspend fun check(
    ...
    for (prefix in config.proxyPrefixes) {
      ...
      try {
        ...
        val release =
          runCatching { GITHUB_JSON.decodeFromString<GhRelease>(resp.body) }
            .getOrNull()
