...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    ...
    val initial =
      buildNotification(
        title = nativeString("DeepSeekHarness Node"),
    ...
    runtimeRestoreJob =
      scope.launch(Dispatchers.Default) {
        try {
          ...
          Log.e("DeepSeekHarnessNodeService", "Failed to restore node runtime", err)
          withContext(Dispatchers.Main) {
            runtimeRestoreJob = null
          ...
          val title =
            when {
              state.connected && state.mode == VoiceCaptureMode.TalkMode ->
                nativeString("DeepSeekHarness Node · Talk")
              state.connected -> nativeString("DeepSeekHarness Node · Connected")
              else -> nativeString("DeepSeekHarness Node")
            }
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = nativeString("DeepSeekHarness node connection status")
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.deepseek.harness.action.STOP"
    private const val ACTION_RESUME = "ai.deepseek.harness.action.RESUME"
    private const val ACTION_SET_VOICE_CAPTURE_MODE = "ai.deepseek.harness.action.SET_VOICE_CAPTURE_MODE"
    private const val EXTRA_VOICE_CAPTURE_MODE = "ai.deepseek.harness.extra.VOICE_CAPTURE_MODE"
